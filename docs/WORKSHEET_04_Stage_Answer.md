...
### ⑥: `Length`
- ベクトルの長さを計算
- `Math2D::Length(v)` = \(|$\\vec{v}$| = $\\sqrt{$v_{x}$^2 + $v_{y}$^2}$\)
- 差ベクトルの長さ = 2点間の距離
...
2点 \(A($x_{1}$, $y_{1}$)\) と \(B($x_{2}$, $y_{2}$)\) の距離 \(d\) は：

$
d = $\\sqrt{($x_{2}$ - $x_{1}$)^2 + ($y_{2}$ - $y_{1}$)^2}$
$

差ベクトル \($\\vec{r}$ = B - A = ($x_{2}$ - $x_{1}$, $y_{2}$ - $y_{1}$)\) の長さは：

$
|$\\vec{r}$| = $\\sqrt{$r_{x}$^2 + $r_{y}$^2}$ = $\\sqrt{($x_{2}$ - $x_{1}$)^2 + ($y_{2}$ - $y_{1}$)^2}$
$
