...
### 📋 パート1: ヘッダーファイル（Target.h）の穴埋め【最初にやる】
...
💡 **重要:** これが最後の.hファイルです！
   このパート1が完了したら、初めてビルドしてみましょう。

---

### 🎉 パート1（3つのワークシート）完了後の確認

ワークシート01〜03のパート1がすべて完了したら：

1. **ビルドしてみる**
   - Visual Studioで「ビルド」→「ソリューションのビルド」
...
3. **この時点では動きません**
   - 砲台は表示されるが動かない
   - これは正常です（.cppがまだ未実装のため）

---

### 🔧 パート2: ソースファイル（Target.cpp）の穴埋め【パート1完了後】
...
### 【パート1完了】Target.h の穴埋めが終わったら

- [ ] `Target.h` に構文エラー（赤い波線）がない
- [ ] `/* TODO: */` コメントをすべて埋めた
- [ ] 親クラス名と変数名が正しい

---

### 🎉 フェーズ1完了確認（ワークシート01〜03のパート1）

すべての.hファイルが完成したら、ビルドしてみましょう：
...
✅ **ここまで完了したら、フェーズ1（ビルドできるようにする）達成です！**

💡 **次は:** フェーズ2（機能実装）に進みます。ワークシート01のパート2から始めましょう。

---

### 【パート2完了】Target.cpp の実装が終わったら
...
💡 **次のステップ:**  
このワークシートが完成したら、次は **ワークシート04** に進みましょう！
