...
このワークシートは **2つのパート** に分かれています：

### 📋 パート1: ヘッダーファイル（Cannon.h）の穴埋め【最初にやる】

**対象ファイル:** `Cannon.h`  
**目的:** クラス定義を完成させて、コンパイルエラーを解消する  
**所要時間:** 約10分

**実装箇所:**
- 問題1: クラス定義の穴埋め

💡 **重要:** まずこのパート1を完成させてください。
   すべてのワークシート（01〜03）のパート1が終わったら、ビルドしてエラーがないか確認します。

---

### 🔧 パート2: ソースファイル（Cannon.cpp）の穴埋め【パート1完了後】
...
**実装箇所:**
- 問題2: コンストラクタの穴埋め（提供済み）
- 問題3: GetDirection() の実装
- 問題4: GetTipPosition() の実装
...
角度 $ \theta $（ラジアン）から、長さ 1 の方向ベクトル $ \vec{d} $ を求める公式：
...
### 例題
- $ \theta = 0^\circ = 0 \,\text{rad} $  
  $(\cos 0,\ \sin 0) = (1, 0)$ → 右向き

- $ \theta = 45^\circ = \pi/4 \,\text{rad} $  
  $(\cos(\pi/4),\ \sin(\pi/4)) = (0.707,\ 0.707)$ → 右斜め上

- $ \theta = 90^\circ = \pi/2 \,\text{rad} $  
  $(\cos(\pi/2),\ \sin(\pi/2)) = (0, 1)$ → 真上


## 2. ベクトルのスカラー倍

ベクトル $ \vec{v} $ にスカラー（数値） $ s $ を掛ける：

$
s \cdot \vec{v} = (s v_x,\ s v_y)
$
...
### 【パート1完了】Cannon.h の穴埋めが終わったら

- [ ] `Cannon.h` に構文エラー（赤い波線）がない
- [ ] `/* TODO: */` コメントをすべて埋めた
- [ ] 親クラス名と変数名が正しい

💡 **次のステップ:** 
- まだビルドしないでください
- ワークシート02、03のパート1も完成させましょう
- 3つすべて完了したらビルドします

---

### 【パート2完了】Cannon.cpp の実装が終わったら

- [ ] `GetDirection()` を実装した
- [ ] `GetTipPosition()` を実装した
- [ ] コンパイルエラーがない
- [ ] ビルドして実行できる
- [ ] 左右キーで砲台の角度が変わる
- [ ] 砲身が正しい方向を向いている
...
💡 **次のステップ:**  
このワークシートが完成したら、次は **ワークシート02** に進みましょう！
